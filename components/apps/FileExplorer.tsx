...
import {
  ...
  Code, Archive, ArrowUpDown, Star, RefreshCw
} from 'lucide-react';
...
function kernelStatToFile(stat: KernelFileStat, parentPath: string): FileSystemItem {
  ...
  else if (['ts', 'js', 'tsx', 'jsx', 'py', 'rs', 'go', 'c', 'cpp', 'h', 'java', 'sh', 'json', 'yaml', 'toml', 'css', 'html', 'sql', 'rb', 'php'].includes(ext)) kind = 'code';
  ...
  return (
    <div className="flex h-full bg-white/80 backdrop-blur-xl text-gray-800 font-sans">
      ...
      <div className="w-48 bg-gray-100/50 border-r border-gray-200/50 p-3 flex flex-col gap-1 backdrop-blur-md">
        ...
        {useKernel ? (
          <>
            <button onClick={() => navigate('/home')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-colors ${kernelPath === '/home' ? 'bg-black/5 font-medium' : 'hover:bg-black/5 text-gray-600'}`}>
              <Home size={16} className="text-blue-500" /> Home
            </button>
            <button onClick={() => navigate('/home/root')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-colors ${kernelPath === '/home/root' ? 'bg-black/5 font-medium' : 'hover:bg-black/5 text-gray-600'}`}>
              <Monitor size={16} className="text-teal-500" /> Root
            </button>
            <button onClick={() => navigate('/home/root/Documents')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-colors ${kernelPath.includes('/Documents') ? 'bg-black/5 font-medium' : 'hover:bg-black/5 text-gray-600'}`}>
              <FileText size={16} className="text-blue-500" /> Documents
            </button>
            <button onClick={() => navigate('/home/root/Projects')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-colors ${kernelPath.includes('/Projects') ? 'bg-black/5 font-medium' : 'hover:bg-black/5 text-gray-600'}`}>
              <Code size={16} className="text-green-600" /> Projects
            </button>
            <button onClick={() => navigate('/tmp')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-colors ${kernelPath === '/tmp' ? 'bg-black/5 font-medium' : 'hover:bg-black/5 text-gray-600'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="h-12 border-b border-gray-200/50 flex items-center px-4 justify-between bg-white/40">
          <div className="flex items-center gap-4">
            ...
            <span className="font-semibold text-sm text-gray-700 truncate max-w-[300px]">{getBreadcrumbs()}</span>
            {useKernel && (
              <button onClick={() => loadKernelDir(kernelPath)} className="p-1 rounded hover:bg-black/5 transition-all text-gray-400 hover:text-gray-700" title="Refresh">
